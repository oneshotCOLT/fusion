...
function start()
	...
	menu = gg.choice({
		...
		"[💫] Hechizos",
		"[🎉] Diversion!",
		"[💨] Quitar muros de viento",
		"[☀️] Absorber velas",
		"[✨] Autoquemar y mas hacks",
		"[⚙️] Configuracion"
		-- "!!! {Debug Features}"
	}, nil, "Creado por Meph:")
	...
	if menu == nil then
		...
		elseif menu == 2 then
			...
			if cmode == 1 then
				...
				if family ~= nil then
					if gg.alert("nota : Farm semi-auto presionando el icono GG. ¿Vamos por velas en "..get_map_name().."?", "SI", "Cancelar") == 1 then
						DoPoints(make_points_list(map))
					else
						return
					end
					...
					if map == "CandleSpace" then
						gg.toast("Estas en home,entra a un mapa :)!")
					else
						gg.toast("aun no sirve buscar alas en este mapa "..get_map_name())
					end
				end

			elseif cmode == 2 then
				gg.toast("Proximamente..")
			end
		...
		elseif menu == 3 then
			...
			if tpmenu == 1 then
				...
				local mpchoice = gg.choice(mps, nil, "Elije un mapa para viajar")
		...
		elseif menu == 5 then
			if cosmetic_lock == on then
				gg.toast("No puede usar Capas mientras están habilitados los cosmeticos (será posible en futuras actualizaciones)).")
				return
			end
		...
		elseif menu == 7 then
			funmenu = gg.multiChoice({
				inffire.." Fuegos artificiales infinitos 🎆",
				fakesleep.." Dormir falso 💤",
			}, nil, "Cosas divertidas:")
		...
		elseif menu == 10 then
			hackmenu = gg.multiChoice({
				autoburn.." Autoquemado 🔥",
				cosmetics.." Desbloquear todos los Cosmeticos y Emotes 🔓",
				friendnode_unlock.." desbloquear amistad con todo mundo 🔓",
				energy.." Energuia infinita ♾️",
				quick.." Quick Steps ⚡",
				clouds.." Quitar nubes ☁️",
				noknock.." Que no te Derriben 🚹"
				-- cosmetic_lock.." Cosmetic Lock"
			}, nil, "Selecciona Hacks:")
			...
			if hackmenu == nil then
				...
				if hackmenu[1] then
					if nentity_test then
						...
						gg.toast("AutoQuemar no se puede activar debido a un error interno.")
					end
				end
